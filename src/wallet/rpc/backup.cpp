    ...
    std::vector<ImportDescriptorRequest>& requests)
{
    ...
    if (!reserver.reserve(/*with_passphrase=*/true)) {
        return make_error(WalletErrorCode::GenericError,
    ...
    bool rescan = false;
    {
        ...
        if (wallet.IsLocked()) {
            return make_error(WalletErrorCode::UnlockNeeded,
    ...
    if (rescan) {
        ...
        if (wallet.IsAbortingRescan()) {
            return make_error(WalletErrorCode::MiscError,
        ...
        [](const RPCMethod& self, const JSONRPCRequest& main_request) -> UniValue
{
    ...
    for (size_t i = 0; i < univalue_requests.size(); ++i) {
        ...
        try {
            requests.push_back(ProcessUniValueDescriptor(univalue_requests[i], timestamp));
        } catch (const UniValue& e) {
            parse_errors[i] = e;
        }
    }

    std::vector<ImportResult> import_results{ProcessDescriptorsImport(wallet, requests)};
    ...
    if (import_results.size() == 1 && import_results[0].has_error() && import_results[0].error->is_general_error) {
        ...
        switch (import_error.wallet_error.code) {
